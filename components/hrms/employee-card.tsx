    ...
    <div className="employee-card group border-slate-200/80 bg-white/95 shadow-sm shadow-slate-200/70 transition-all duration-200 hover:-translate-y-0.5 hover:border-primary/20 hover:shadow-lg hover:shadow-primary/70 dark:border-slate-800 dark:bg-slate-900/85 dark:shadow-slate-950/30 dark:hover:border-primary/40 dark:hover:shadow-primary/30">
      <button type="button" className="block w-full p-5 pb-3 text-start" onClick={() => onView(employee.id)}>
        <div className="flex items-start gap-4">
          {employee.profilePhotoUrl ? (
            ...
            <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-primary/8 to-violet-50 text-primary font-semibold text-lg ring-2 ring-white shadow-md shadow-slate-200/80 transition-transform duration-200 group-hover:scale-[1.03] dark:from-primary/50 dark:to-violet-950/40 dark:ring-slate-900 dark:shadow-slate-950/40">{initials}</div>
          )}
      ...
      <div className="border-t border-slate-100 bg-slate-50/50 px-3 py-2.5 flex items-center justify-between dark:border-slate-800 dark:bg-slate-900/50">
        <div className="flex items-center gap-1">
          <Button variant="ghost" size="sm" className="h-8 gap-1.5 text-xs text-slate-600 hover:bg-primary/8 hover:text-primary dark:text-slate-300 dark:hover:bg-primary/40 dark:hover:text-primary/30" onClick={() => onEdit(employee.id)}>
            <Eye className="h-3.5 w-3.5" />{locale === "ar" ? "الملف الكامل" : "Full Profile"}
          </Button>
          <Button variant="ghost" size="sm" className="h-8 gap-1.5 text-xs text-slate-600 hover:bg-primary/8 hover:text-primary dark:text-slate-300 dark:hover:bg-primary/40 dark:hover:text-primary/30" onClick={() => onDocuments(employee.id)}>
