        ...
        <div className="flex min-h-16 items-center justify-between gap-4 px-4 lg:px-8">
          ...
          <div className="flex min-w-0 flex-1 items-center justify-center px-4">
            <div className="hidden max-w-xl flex-1 items-center gap-2 rounded-full border bg-background px-4 py-2 text-sm text-muted-foreground shadow-sm md:flex">
              <Sparkles className="h-4 w-4 text-primary" /> {dictionary.nav.workspace}
            </div>
          </div>
          <div className="flex items-center gap-2">
            <LanguageSwitcher locale={locale} />
            <Button type="button" variant="outline" size="icon" aria-label={dictionary.common.notifications}><Bell className="h-4 w-4" /></Button>
            <ThemeToggle />
            <form action={logoutAction}>
              <Button type="submit" variant="outline" className="gap-2"><LogOut className="h-4 w-4" /> <span className="hidden sm:inline">{dictionary.common.signOut}</span></Button>
            </form>
          </div>
        </div>
      </header>
      <div className="grid lg:grid-cols-[292px_1fr]">
        <aside className="border-b bg-background/80 backdrop-blur lg:min-h-[calc(100vh-4rem)] lg:border-b-0 lg:border-r">
          <div className="border-b p-4">
            <p className="text-xs font-medium uppercase text-muted-foreground">{dictionary.common.signedInAs}</p>
