...
export async function BiometricLogsBody() {
  const logs = await getBiometricLogs();

  return (
    <section className="space-y-6">
      <WorkspaceHeader title="سجلات أجهزة الحضور" description="آخر 50 حركة دخول وخروج مستلمة من أجهزة الحضور." actions={<BiometricLogsRefresher />} />
      ...
      <Card>
        ...
        <CardContent>
          <div className="overflow-x-auto">
              ...
              <tbody>
                {logs.map(({ record, parsed }) => (
                  <tr key={record.id} className="border-b last:border-0">
                    ...
                    <td className="p-2 text-muted-foreground" title={parsed.deviceIp ?? undefined}>{parsed.deviceName ?? "جهاز الحضور"}</td>
            ...
            {!logs.length && <div className="rounded-xl border border-dashed p-8 text-center text-muted-foreground">لا توجد حركات حضور حتى الآن.</div>}
          </div>
        </CardContent>
      </Card>
    </section>
