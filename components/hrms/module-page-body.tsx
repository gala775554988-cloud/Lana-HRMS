    ...
    <section className="space-y-5" dir={locale === "ar" ? "rtl" : "ltr"}>
      ...
      <div className="flex flex-col gap-3 rounded-2xl border bg-card px-4 py-3 shadow-premium-sm sm:flex-row sm:items-center sm:justify-between">
        ...
        <ModuleToolbarMenu
          ...
          search={search}
          showModuleTabs={showModuleTabs}
          openReportsLabel={t.openReports}
          exportFields={resource.fields.map((field) => ({ name: field.name, label: field.label }))}
          showImport={resource.key !== "audit-logs"}
          importFieldNames={resource.fields.map((field) => field.name).join(", ")}
        />
      </div>
