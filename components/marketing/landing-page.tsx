...
export function LandingPage({ locale }: { locale: Locale }) {
  ...
  return (
    <main dir={dir} className="min-h-screen overflow-x-hidden bg-slate-50 dark:bg-slate-950">
      <div className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
        <div className="absolute -top-32 end-[-10%] h-[520px] w-[520px] rounded-full bg-primary/10 blur-[140px]" />
        <div className="absolute bottom-[-15%] start-[-10%] h-[420px] w-[420px] rounded-full bg-secondary/10 blur-[140px]" />
      </div>
      ...
      <section className="relative px-4 pb-20 pt-16 sm:px-6 sm:pt-24 lg:px-8 lg:pt-28">
        <div className="mx-auto flex max-w-4xl flex-col items-center text-center">
          ...
          <h1 className="animate-in fade-in slide-in-from-bottom-3 mt-6 text-4xl font-extrabold tracking-tight text-slate-900 duration-500 delay-100 fill-mode-both dark:text-slate-50 sm:text-5xl lg:text-6xl">
            {t.heroTitle}
          </h1>
          ...
          <p className="animate-in fade-in slide-in-from-bottom-3 mt-5 max-w-2xl text-base leading-relaxed text-slate-500 duration-500 delay-200 fill-mode-both dark:text-slate-400">
            {t.heroSubtitle}
          </p>
          <div className="animate-in fade-in slide-in-from-bottom-3 mt-8 flex flex-col items-center gap-3 duration-500 delay-300 fill-mode-both sm:flex-row">
            ...
            <a
              href="#features"
              className="inline-flex h-12 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white/70 px-7 text-sm font-bold text-slate-700 backdrop-blur-md transition-all duration-300 hover:-translate-y-0.5 hover:border-primary/40 hover:text-primary active:scale-95 dark:border-slate-800 dark:bg-slate-900/70 dark:text-slate-200"
            >
              {t.ctaSecondary}
            </a>
          </div>
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 mx-auto mt-16 grid max-w-4xl grid-cols-2 gap-4 duration-500 delay-500 fill-mode-both sm:grid-cols-4 sm:gap-6">
          {t.stats.map((stat) => (
            <div
              key={stat.label}
              className="rounded-2xl border border-slate-200/90 bg-white/70 p-4 text-center shadow-glass backdrop-blur-md dark:border-slate-800 dark:bg-slate-900/70"
            >
              <span className="block text-2xl font-extrabold text-slate-900 dark:text-slate-50 sm:text-3xl">{stat.value}</span>
              <span className="mt-1 block text-xs font-medium text-slate-500 dark:text-slate-400">{stat.label}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="features" className="px-4 py-16 sm:px-6 sm:py-20 lg:px-8">
        <div className="mx-auto max-w-6xl">
          <div className="mx-auto max-w-2xl text-center">
            <h2 className="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-slate-50 sm:text-4xl">{t.featuresTitle}</h2>
            <p className="mt-3 text-base leading-relaxed text-slate-500 dark:text-slate-400">{t.featuresSubtitle}</p>
          </div>
            ...
            {t.features.map((feature, index) => {
              ...
              return (
                <div
                  ...
                  className="animate-in fade-in slide-in-from-bottom-3 group rounded-2xl border border-slate-200/90 bg-white/70 p-6 shadow-glass backdrop-blur-md duration-500 fill-mode-both transition-all hover:-translate-y-1 hover:border-primary/40 hover:shadow-xl hover:shadow-primary/10 dark:border-slate-800 dark:bg-slate-900/70"
                >
                  ...
                  <h3 className="mt-4 text-base font-bold text-slate-900 dark:text-slate-50">{feature.title}</h3>
                  <p className="mt-1.5 text-sm leading-relaxed text-slate-500 dark:text-slate-400">{feature.desc}</p>
                </div>
              );
            })}
          </div>
      ...
      <section className="px-4 pb-20 sm:px-6 lg:px-8">
        <div className="relative mx-auto max-w-6xl overflow-hidden rounded-3xl bg-[#030C1B] px-6 py-14 text-center text-white shadow-2xl sm:px-12 sm:py-16">
          <div className="absolute -top-24 end-[-5%] h-72 w-72 rounded-full bg-primary/20 blur-[110px]" />
          <div className="absolute -bottom-24 start-[-5%] h-72 w-72 rounded-full bg-secondary/20 blur-[110px]" />
          <div className="relative z-10 mx-auto flex max-w-xl flex-col items-center gap-4">
            ...
            <h2 className="text-2xl font-extrabold sm:text-3xl">{t.ctaBandTitle}</h2>
            <p className="text-sm font-light text-slate-300 sm:text-base">{t.ctaBandSubtitle}</p>
            <Link
              href="/login"
              className="mt-2 inline-flex h-12 items-center justify-center gap-2 rounded-xl bg-primary px-8 text-sm font-bold text-primary-foreground shadow-lg shadow-primary/30 transition-all duration-300 hover:-translate-y-0.5 hover:bg-primary/90 active:scale-95"
            >
              {t.ctaBandButton}
              <NextIcon className="h-4 w-4" aria-hidden="true" />
            </Link>
      ...
      <footer className="border-t border-slate-200/70 px-4 py-8 sm:px-6 lg:px-8 dark:border-slate-800">
