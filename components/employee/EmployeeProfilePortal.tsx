...
export function EmployeeProfilePortal({ employee, settings }: Props) {
  ...
  return <main className="space-y-6" dir="rtl">
    ...
    {tab === 'personal' && <Card className="rounded-3xl"><CardHeader><CardTitle>المعلومات الشخصية</CardTitle></CardHeader><CardContent className="grid gap-4 md:grid-cols-3"><Field label="الاسم العربي" value={fullName}/><Field label="الاسم بالإنجليزية" value={employee.employeeEnglishName}/><Field label="رقم الهوية أو الإقامة" value={employee.nationalId}/><label className="space-y-1"><span className="text-sm text-muted-foreground">الجوال</span><Input value={personal.phone} onChange={e=>setPersonal({...personal, phone:e.target.value})}/></label><Field label="هاتف العمل" value={employee.workPhone}/><Field label="الجوال من أودو" value={employee.mobilePhone}/><label className="space-y-1"><span className="text-sm text-muted-foreground">البريد</span><Input value={personal.email} onChange={e=>setPersonal({...personal, email:e.target.value})}/></label><Field label="الجنسية" value={employee.nationality?.name}/><label className="space-y-1"><span className="text-sm text-muted-foreground">الجنس</span><Input value={personal.gender} onChange={e=>setPersonal({...personal, gender:e.target.value})}/></label><label className="space-y-1"><span className="text-sm text-muted-foreground">تاريخ الميلاد</span><Input type="date" value={personal.dateOfBirth} onChange={e=>setPersonal({...personal, dateOfBirth:e.target.value})}/></label><Field label="الحالة الاجتماعية" value={employee.maritalStatus}/><Field label="جهة اتصال الطوارئ" value={employee.emergencyContact}/><label className="space-y-1 md:col-span-3"><span className="text-sm text-muted-foreground">العنوان</span><Input value={personal.address} onChange={e=>setPersonal({...personal, address:e.target.value})}/></label><div className="md:col-span-3"><Button disabled={pending} onClick={savePersonal}><Save className="ml-2 h-4 w-4"/>حفظ البيانات الشخصية</Button></div></CardContent></Card>}
    {tab === 'job' && <Card className="rounded-3xl"><CardHeader><CardTitle>البيانات الوظيفية</CardTitle></CardHeader><CardContent className="grid gap-4 md:grid-cols-3"><Field label="الرقم الوظيفي" value={employee.employeeNumber}/><Field label="القسم" value={employee.department?.name}/><Field label="الفرع" value={employee.branch?.name}/><Field label="المستشفى أو موقع العمل" value={employee.workLocationName}/><Field label="المدير المباشر" value={employee.manager ? `${employee.manager.firstName} ${employee.manager.lastName}` : ''}/><Field label="المسمى الوظيفي" value={employee.position?.title}/><Field label="مهنة الإقامة" value={employee.iqamahJobName}/><Field label="نوع العقد" value={employee.employmentType?.name}/><Field label="الكفيل" value={employee.sponsor}/><Field label="مركز التكلفة" value={employee.costCenter}/><Field label="حالة العمل" value={employee.workingStatus}/><Field label="تاريخ التعيين" value={employee.hireDate ? new Date(employee.hireDate).toLocaleDateString('ar-SA') : null}/><Field label="تاريخ أول عقد" value={employee.firstContractDate ? new Date(employee.firstContractDate).toLocaleDateString('ar-SA') : null}/></CardContent></Card>}
