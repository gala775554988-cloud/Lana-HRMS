...
export async function POST(request: NextRequest) {
  try {
    ...
    if (newPassword.length < 8) {
      return NextResponse.json({ success: false, message: "كلمة المرور يجب أن تكون 8 أحرف على الأقل" }, { status: 400 });
    }
    ...
    if (currentPassword === newPassword) {
      return NextResponse.json({ success: false, message: "كلمة المرور الجديدة يجب أن تكون مختلفة" }, { status: 400 });
    }
