 ...
 * most recent runs, missing-data detection (active employees with no
 * contract / no salary amount, so payroll can't compute them), upcoming
 * payroll periods, recent run activity from the existing AuditLog, and a
 * department/branch cost breakdown of the most recent run. Every number here
 * is a direct aggregate query against tables the payroll engine already
 * reads/writes -- nothing new is stored just for this dashboard. */
export async function GET() {
  ...
  const [statusGroups, missingContracts, missingSalaryAmount, upcomingPeriods, recentActivityRaw, latestRun] = await Promise.all([
  ...
  const statusCounts: Record<string, number> = { DRAFT: 0, PROCESSING: 0, APPROVED: 0, PAID: 0, CANCELLED: 0 };
  ...
  let latestRunTotals = { gross: 0, net: 0, employeeCount: 0 };
