      ...
      <div className="relative z-10 mx-auto w-full max-w-5xl space-y-8">
        ...
        <div className="grid items-center gap-8 xl:grid-cols-[0.85fr_1.15fr]">
          <div className="space-y-5">
            <h2 className="text-5xl font-black leading-tight tracking-tight xl:text-6xl">
              ...
              <span className="block text-white/80">ومنصة واحدة.</span>
            </h2>
            <p className="max-w-xl text-base leading-8 text-white/70 xl:text-lg">
          ...
          <div className="relative min-h-[520px]">
            ...
            <div className="absolute left-1/2 top-5 z-20 h-[500px] w-[250px] -translate-x-1/2 rounded-[3rem] border border-white/40 bg-[#081B46] p-3 shadow-2xl shadow-[#081B46]/40 [animation:phoneFloat_7s_ease-in-out_infinite]">
              <div className="h-full overflow-hidden rounded-[2.35rem] bg-[#F7F9FC] p-4 text-[#111827]">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs font-bold text-[#64748B]">Lana HRMS</p>
                    <h3 className="text-xl font-black text-[#262B63]">{phoneSlides[0]}</h3>
                  </div>
                  <div className="grid h-11 w-11 place-items-center rounded-2xl bg-[#3F46E8] text-white"><Smartphone className="h-5 w-5" /></div>
                </div>
                <div className="mt-5 rounded-3xl bg-gradient-to-br from-[#3F46E8] to-[#262B63] p-5 text-white shadow-xl">
                  <p className="text-sm text-white/70">Mobile demo</p>
                  <div className="mt-4 text-5xl font-black">360°</div>
                  <div className="mt-5 space-y-2">
                    {phoneSlides.slice(0, 3).map((item) => <div key={item} className="rounded-2xl bg-white/15 px-3 py-2 text-sm backdrop-blur">{item}</div>)}
                  </div>
                </div>
                <div className="mt-4 grid grid-cols-2 gap-3">
                  {phoneSlides.slice(2).map((item, index) => (
                    <div key={item} className="rounded-2xl border border-[#E5E7EB] bg-white p-3 shadow-sm">
                      <div className="mb-2 h-2 w-10 rounded-full bg-[#3F46E8]/20" />
                      <p className="text-[10px] font-bold text-[#64748B]">{item}</p>
                      <p className="mt-1 text-lg font-black text-[#262B63]">{(index + 2) * 8}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {[
              ...
              <div key={label} className={`absolute z-30 rounded-2xl border border-white/25 bg-white/12 px-3 py-2 text-xs font-black text-white shadow-xl backdrop-blur-xl ${index % 2 ? "left-4" : "right-4"}`} style={{ top: `${14 + index * 13}%`, animation: `phoneFloat ${6 + index}s ${index * 0.35}s ease-in-out infinite` }}>
                {label}
              </div>
            ))}
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 xl:grid-cols-6">
          {stats.map((stat, index) => {
            ...
            return (
              <div key={stat.label} className="rounded-3xl border border-white/20 bg-white/10 p-4 shadow-xl shadow-[#081B46]/15 backdrop-blur-xl transition duration-300 hover:-translate-y-1 hover:scale-[1.02] hover:bg-white/15" style={{ animation: `lanaSlideUp 520ms ${index * 65}ms ease both` }}>
                <div className="mb-3 grid h-10 w-10 place-items-center rounded-2xl bg-white/15"><Icon className="h-5 w-5" /></div>
...
export default async function LoginPage({
  ...
  const loginPanel = (
    <section className="relative flex min-h-screen items-center justify-center bg-slate-50/90 p-6" dir={isAr ? "rtl" : "ltr"}>
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white/95 rounded-3xl shadow-xl border p-8 backdrop-blur">
          ...
          <div className="mt-6 pt-5 border-t text-center">
            {!isAdminMode ? (
              <Link href="/login?admin=true" className="inline-flex items-center gap-1.5 text-xs text-slate-500 hover:text-purple-600 transition-colors">
                <span>👑</span>
                <span>دخول المسؤولين (لوحة التحكم)</span>
              </Link>
            ) : (
              <Link href="/login" className="inline-flex items-center gap-1.5 text-xs text-slate-500 hover:text-indigo-600 transition-colors">
                <span>👤</span>
