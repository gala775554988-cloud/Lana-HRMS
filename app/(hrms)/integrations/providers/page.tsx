...
export default async function ProvidersPage() {
  ...
  return <IntegrationShell title="المزوّدون" description="مزوّدو أنظمة ERP مثل Odoo."><DataCard title="إضافة مزوّد"><form action={createProvider} className="grid gap-3 md:grid-cols-5"><Input name="name" placeholder="Odoo Production" required /><Input name="code" placeholder="odoo-prod" required /><Input name="type" defaultValue="ODOO" /><Input name="baseUrl" placeholder="https://odoo.example.com" required /><Button type="submit">إنشاء</Button></form></DataCard><DataCard title="المزوّدون"><SimpleTable columns={["name", "code", "type", "baseUrl", "isActive", "createdAt"]} rows={rows as unknown as Array<Record<string, unknown>>} /></DataCard></IntegrationShell>;
}
