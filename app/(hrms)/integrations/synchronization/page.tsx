...
export default async function SynchronizationPage() {
  ...
  const form = (action: (formData: FormData) => Promise<void>, label: string) => (
    <form action={action} className="grid gap-3 md:grid-cols-4">
      ...
      <select name="direction" className="h-10 rounded-md border bg-background px-3">
        <option value="BIDIRECTIONAL">كلا الاتجاهين</option>
  ...
  return (
    <IntegrationShell title="مزامنة Odoo 🔄" description="صفحة كاملة تحتوي حالة الاتصال، آخر مزامنة، عدد الموظفين، الإدارات، الفروع، العقود، الأخطاء، إعادة المزامنة، جدولة المزامنة، Logs - متوافقة بالكامل مع Odoo">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <DataCard title="حالة الاتصال">
          <div className="space-y-2">
            ...
            <div className="text-xs">الرابط الأساسي: {(connections[0] as any)?.baseUrl || "-"}</div>
            <div className="text-xs">آخر اختبار: {(connections[0] as any)?.lastTestAt ? new Date((connections[0] as any).lastTestAt).toLocaleString("ar-SA") : "-"}</div>
          </div>
        </DataCard>
        ...
        <DataCard title="آخر مزامنة">
          <div className="space-y-2">
            ...
            <div className="text-xs">المسحوبة: {lastSync?.pulled || 0} | المُرسلة: {lastSync?.pushed || 0}</div>
          </div>
        </DataCard>
      ...
      <DataCard title="إعادة المزامنة وجدولة المزامنة">
        <div className="space-y-4">
          ...
          <div className="text-sm text-muted-foreground">
            <p>• <strong>إعادة المزامنة:</strong> استخدم زر تشغيل الآن أدناه مع اتجاه ODOO_TO_HRMS و Batch 500</p>
            <p>• <strong>جدولة المزامنة:</strong> يتم عبر جدولة المزامنة الخلفية - يعمل كل 10 دقائق + يومياً 2AM عبر Cron</p>
            <p>• <strong>السجلات:</strong> تظهر أدناه مع الأخطاء والتفاصيل</p>
          </div>
        </div>
      </DataCard>
          ...
          {lastErrors.length === 0 ? (
            ...
            lastErrors.map((err: any) => (
              <div key={err.id} className="rounded-lg border border-red-200 bg-red-50 p-3 text-sm">
                <p className="font-bold text-red-800">{err.entity} - {err.status} - {err.error?.slice(0,200) || "لا توجد رسالة خطأ"}</p>
                <p className="text-xs text-muted-foreground mt-1">{err.startedAt ? new Date(err.startedAt).toLocaleString("ar-SA") : ""} - المسحوبة: {err.pulled}, تم الإنشاء: {err.createdCount}, تم التخطي: {(err.metadata as any)?.skipped || 0}</p>
              </div>
            ))
          )}
        </div>
      ...
      <DataCard title="تشغيل المزامنة الآن (إعادة المزامنة)">{form(runSync, "تشغيل الآن - مزامنة فورية")}</DataCard>
      <DataCard title="جدولة المزامنة الخلفية (جدولة)">{form(queueSync, "جدولة المهمة - جدولة في الخلفية")}</DataCard>

      <DataCard title="سجل المزامنة (آخر 30 مزامنة)"><SimpleTable columns={["startedAt", "finishedAt", "entity", "direction", "status", "pulled", "pushed", "createdCount", "conflictCount", "error"]} rows={history as unknown as Array<Record<string, unknown>>} /></DataCard>

      <DataCard title="السجلات (آخر 50 سجل)">
      ...
      <DataCard title="التعارضات المعلّقة"><SimpleTable columns={["createdAt", "entity", "localId", "externalId", "field", "resolution"]} rows={conflicts as unknown as Array<Record<string, unknown>>} /></DataCard>
    </IntegrationShell>
  );
}
