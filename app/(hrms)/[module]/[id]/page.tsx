...
export default async function RecordPage({ params }: { params: Promise<{ module: string; id: string }> }) {
  ...
  return (
    <section className="space-y-6">
      <div className="flex items-center justify-between gap-4"><div><p className="text-sm font-medium text-muted-foreground">{resource.title}</p><h1 className="text-3xl font-semibold">Record profile</h1></div><Button asChild variant="outline"><Link href={"/" + resource.key}>Back</Link></Button></div>
      <div className="grid gap-6 xl:grid-cols-[1fr_420px]">
        ...
        <Card><CardHeader><CardTitle>Edit</CardTitle><CardDescription>Changes are validated and audited.</CardDescription></CardHeader><CardContent><ModuleForm resource={resource} initialValues={record} recordId={id} /></CardContent></Card>
      </div>
