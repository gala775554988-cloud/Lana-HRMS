...
function PrintReportContent() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/enterprise/social-insurance/export?${params.toString()}`, { cache: "no-store" })
      .then((r) => r.json())
  ...
  return (
    <div className="mx-auto max-w-5xl p-8 print:p-0" dir="rtl">
      ...
      <div className="mb-6 border-b pb-4">
        <h1 className="text-2xl font-black">{title || "تقرير التأمينات الاجتماعية"}</h1>
          ...
          <tbody>
            {rows.map((row, i) => (
              <tr key={i} className="border-b border-slate-200">
                {columns.map((col) => (
                  <td key={col} className="p-2 text-right">{String(row[col] ?? "")}</td>
                ))}
              </tr>
            ))}
          </tbody>
...
export default function SocialInsurancePrintReportPage() {
  return (
