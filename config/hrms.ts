...
const employeeIdField: ModuleField = { name: "employeeId", label: "الموظف", type: "text", required: true };
...
export const hrmsModules = [
  { key: "hospitals", title: "المستشفيات", description: "إدارة المستشفيات والفروع", model: "hospital", permissionResource: "hospitals", searchFields: ["name", "code"], filterFields: ["isActive"], tableFields: ["code", "name", "isActive"], fields: [{ name: "name", label: "Name", type: "text", required: true }] },
  {
    ...
    fields: [
      ...
      { name: "phone", label: "رقم التواصل", type: "text" },
      { name: "employeeEnglishName", label: "الاسم باللغة الإنجليزية", type: "text" },
      { name: "iqamahJobName", label: "المهنة في الإقامة", type: "text" },
      { name: "mobilePhone", label: "الجوال", type: "text" },
      { name: "workPhone", label: "هاتف العمل", type: "text" },
      { name: "maritalStatus", label: "الحالة الاجتماعية", type: "select", options: ["SINGLE", "MARRIED", "DIVORCED", "WIDOWED"] },
      { name: "emergencyContact", label: "جهة اتصال الطوارئ", type: "text" },
      { name: "gender", label: "Gender", type: "select", options: ["Female", "Male", "Not specified"] },
      { name: "dateOfBirth", label: "Date of birth", type: "date" },
      { name: "hireDate", label: "Hire date", type: "date", required: true },
      { name: "firstContractDate", label: "تاريخ بداية أول عقد", type: "date" },
      { name: "workingStatus", label: "حالة العمل", type: "text" },
  ...
  {
    key: "attendance", title: "الحضور والانصراف", description: "متابعة الدوام الفعلي والتأخير والغياب وربطه بالورديات.", model: "attendanceRecord", permissionResource: "attendance", searchFields: ["notes"], filterFields: ["status", "employeeId"], tableFields: ["employeeName", "employeeNumber", "workDate", "scheduledShift", "checkIn", "checkOut", "workedHours", "lateMinutes", "status"], fields: [employeeIdField, { name: "workDate", label: "تاريخ الدوام", type: "date", required: true }, { name: "checkIn", label: "وقت الحضور", type: "date" }, { name: "checkOut", label: "وقت الانصراف", type: "date" }, { name: "status", label: "الحالة", type: "select", options: ["PRESENT", "ABSENT", "LATE", "HALF_DAY", "REMOTE", "HOLIDAY"] }, { name: "notes", label: "ملاحظات", type: "textarea" }]
  },
  {
    key: "leave-types", title: "Leave Types", description: "Configure paid and unpaid leave policies.", model: "leaveType", permissionResource: "leave", searchFields: ["name", "code"], filterFields: ["isActive", "isPaid", "genderRestriction"], tableFields: ["code", "name", "annualLimit", "isPaid", "genderRestriction", "isActive"], fields: [{ name: "name", label: "Name", type: "text", required: true }, codeField, descriptionField, { name: "annualLimit", label: "Annual limit", type: "number" }, { name: "carryOverLimit", label: "Carry-over limit", type: "number" }, { name: "genderRestriction", label: "Gender restriction", type: "select", options: ["MALE", "FEMALE"] }, { name: "isPaid", label: "Paid", type: "boolean" }, activeField]
  },
  {
    key: "leave-requests", title: "طلبات الإجازة", description: "تقديم طلبات الإجازة ومراجعتها واعتمادها وربطها بالرصيد.", model: "leaveRequest", permissionResource: "leave", searchFields: ["reason", "decisionNote"], filterFields: ["status", "employeeId", "leaveTypeId"], tableFields: ["employeeName", "leaveTypeName", "startDate", "endDate", "days", "status"], fields: [employeeIdField, { name: "leaveTypeId", label: "نوع الإجازة", type: "text", required: true }, { name: "startDate", label: "تاريخ البداية", type: "date", required: true }, { name: "endDate", label: "تاريخ النهاية", type: "date", required: true }, { name: "days", label: "عدد الأيام", type: "number", required: true }, { name: "reason", label: "السبب", type: "textarea" }, { name: "status", label: "الحالة", type: "select", options: ["DRAFT", "PENDING", "APPROVED", "REJECTED", "CANCELLED"] }, { name: "decisionNote", label: "ملاحظة القرار", type: "textarea" }]
  },
  {
  ...
  {
    key: "insurance", title: "التأمين الطبي", description: "إدارة وثائق التأمين الطبي والتغطية والتابعين ومواعيد التجديد.", model: "insurancePolicy", permissionResource: "insurance", searchFields: ["policyNumber", "provider"], filterFields: ["status", "employeeId", "category"], tableFields: ["policyNumber", "employeeName", "provider", "coverageType", "category", "dependentsCount", "endDate", "status"], fields: [employeeIdField, { name: "policyNumber", label: "رقم الوثيقة", type: "text", required: true }, { name: "provider", label: "شركة التأمين", type: "text", required: true }, { name: "coverageType", label: "نوع التغطية", type: "text" }, { name: "category", label: "الفئة", type: "select", options: ["VIP", "A", "B", "C", "NA"] }, { name: "dependentsCount", label: "عدد التابعين", type: "number", integer: true }, { name: "startDate", label: "تاريخ البداية", type: "date", required: true }, { name: "endDate", label: "تاريخ الانتهاء", type: "date", required: true }, { name: "status", label: "الحالة", type: "select", options: ["ACTIVE", "EXPIRED", "CANCELLED"] }, { name: "documentUrl", label: "مستند الوثيقة", type: "text" }, { name: "documentName", label: "اسم المستند", type: "text" }, { name: "notes", label: "ملاحظات", type: "textarea" }]
  }
